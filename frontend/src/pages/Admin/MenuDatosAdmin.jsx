...
function MenuDatosAdmin() {
  ...
  const handleBuscarUsuario = async () => {
    try {
      ...
      if (!token) {
        mostrarMensaje("No estás autenticado. Inicia sesión.", "error");
        return;
      }
      ...
      if (modoUsuario === "eliminar") {
        try {
          await api.delete(`/usuarioDelete/${trimmedUsername}`, {
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          if (error.response) {
            if (error.response.status === 404) {
              ...
              mostrarMensaje(`Error ${error.response.status}: No se pudo eliminar el usuario`, "error");
            }
